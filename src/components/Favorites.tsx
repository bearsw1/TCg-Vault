...
export default function Favorites() {
  ...
  return (
    <div className="space-y-8">
      <header>
        <h2 className="text-3xl font-bold text-stone-900 tracking-tight">Favorites</h2>
        <p className="text-stone-500">Your most prized cards</p>
      </header>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-6">
        {cards.map((card) => (
          <Link key={card.id} to={`/card/${card.id}`}>
            <motion.div
              ...
              className="bg-white rounded-2xl border border-stone-100 shadow-sm overflow-hidden flex flex-col hover:border-emerald-200 hover:shadow-md transition-all h-full"
            >
            ...
            <div className="p-3">
              ...
              <p className="text-[10px] text-stone-400 uppercase font-mono tracking-tighter">{card.setName}</p>
            </div>
          </motion.div>
        </Link>
      ))}
        {cards.length === 0 && (
          <div className="col-span-full py-20 text-center bg-white rounded-3xl border border-dashed border-stone-200">
            ...
            <p className="text-stone-400 font-medium">You haven't favorited any cards yet.</p>
          </div>
        )}
      </div>
    </div>
